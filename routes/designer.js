...
function extractJSON(s) {
  ...
  // Rather than patching each new decorator one-by-one as models invent new
  // ones (including non-Latin scripts), strip ANY run of characters that
  // directly wraps a number in a value position (right after ':', '[', or
  // ',' and right before the next ',', ']', or '}'), keeping only the
  // number — as long as those characters aren't valid JSON syntax
  // themselves (quote, comma, colon, brackets, braces, digit, whitespace,
  // '.', '-'). This only fires when the value doesn't already start with a
  // valid JSON value opener ('"', '{', '[', a digit, or '-'), so it can
  // never touch legitimate strings (including URLs with '&' in them, e.g.
  // "...?w=1080&auto=format"), objects, arrays, or already-valid numbers.
